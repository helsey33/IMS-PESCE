...
import { getWorkshopAll } from "../../../../actions/workshopActions";
...
class Workshop extends Component {
  ...
  componentWillReceiveProps(newProps) {
    const { workshops } = newProps.workshop;
    if (workshops) {
      let workshopSet = [];
      workshops.forEach(data => {
        data.workshopData.forEach(ele => {
          ele.name = data.user.name;
          workshopSet.push(ele);
        });
      });
      this.setState({
        workshopSet,
        workshops: workshopSet
      });
    }
  }
  ...
  searchHandle(e) {
    ...
    const filteredSet = this.state.workshops.filter(data => {
  ...
  render() {
    ...
    const { workshops, loading } = this.props.workshop;
    const { workshopSet } = this.state;

    let workshopContent;
    if (workshops === null || loading) {
      workshopContent = (
        <div className={classes.spinner}>
          <CircularProgress size={50} />
        </div>
      );
    } else if (Object.keys(workshops).length > 0) {
      //For exporting CSV
      const headers = [
        { label: "Name", key: "name" },
                        ...
                        {data.report ? (
                          ...
                          <GridItem md={3} sm={3}>
                            <input
                              ...
                              id="paper"
                              type="file"
                              onChange={this.onReportUpload.bind(
                                this,
                                data._id
                              )}
                            />
      ...
      workshopContent = (
        <GridContainer>
          <GridItem md={8}>
            <div>
              <h4 className={classes.cardTitle}>No details available</h4>
            </div>
          </GridItem>
        </GridContainer>
      );
